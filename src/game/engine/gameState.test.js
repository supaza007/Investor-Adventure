...
describe('Scammer', () => {
  // หาบทที่เจอ Scammer จริงแล้วเดินไปถึงข้อเสนอ
  const reachScam = () => {
    for (let seed = 1; seed < 400; seed++) {
      let s = createInitialState(seed)
      s = gameReducer(s, { type: 'START' })
      s = gameReducer(s, { type: 'SELECT_STYLE', styleId: 'medium' })
      if (!s.eventOrder.includes('scammer')) continue
      while (s.eventOrder[s.chapterIndex] !== 'scammer') {
        if (s.phase === 'allocation') {
    ...
    throw new Error('หา seed ที่เจอ Scammer ไม่ได้')
  }

  test('ข้อเสนอโผล่ที่สเตจ 2 เมื่อเจอ Scammer', () => {
    const s = reachScam()
  ...
  test('ปฏิเสธ = ไม่เสียอะไรเลย · รับ = เสียเงินก้อนนั้นจริง', () => {
