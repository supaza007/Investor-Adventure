...
test('สถานะทางการเงินใช้เงินที่ได้รับจริงและตัดช่วงตรงตามแผน', () => {
  assert.equal(reportAt(99.99).band.id, 'ruined')
  assert.equal(reportAt(100).band.id, 'tight')
  assert.equal(reportAt(149.99).band.id, 'tight')
  assert.equal(reportAt(150).band.id, 'adequate')
  assert.equal(reportAt(299.99).band.id, 'adequate')
  assert.equal(reportAt(300).band.id, 'comfortable')
  assert.equal(reportAt(399.99).band.id, 'comfortable')
  assert.equal(reportAt(400).band.id, 'fire')
})
