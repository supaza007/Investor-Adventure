-- Current game analytics: actual income, financial status and age-adjusted matrix.
-- Legacy columns from the removed random-event mechanic remain for old rows, but
-- this version of submit_game_run no longer reads or writes them.
alter table public.game_sessions
  add column if not exists net_gain numeric,
  add column if not exists net_gain_pct numeric;

alter table public.chapter_events
  add column if not exists income_added numeric not null default 0,
  add column if not exists allocation_before_event jsonb not null default '{}'::jsonb,
  add column if not exists base_asset_returns jsonb not null default '{}'::jsonb,
  add column if not exists age_modifiers jsonb not null default '{}'::jsonb,
  add column if not exists final_asset_returns jsonb not null default '{}'::jsonb;
...
begin
  ...
  insert into public.chapter_events (
    ...
    final_asset_returns, shock_pct, percentile, exposure, concentration,
  ...
  select sid, c.chapter_n, c.event_id, c.event_name, c.income_added,
    ...
    c.final_asset_returns, c.shock_pct, c.percentile, c.exposure,
  ...
  from jsonb_to_recordset(coalesce(payload->'chapters', '[]'::jsonb)) as c(
    ...
    final_asset_returns jsonb, shock_pct numeric, percentile numeric,
